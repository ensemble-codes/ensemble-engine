...
export class DexService {
  ...
  async swap(dexArguments: DexArguments, instance: WorkflowInstanceEntity) {
    console.info(
      `Swapping ${dexArguments.tokenInAmount} ${dexArguments.tokenInAddress} for ${dexArguments.tokenOutAddress} on ${dexArguments.dexName} dex. network: ${dexArguments.network}`,
    );
    const dexData = this.getDexData(dexArguments.dexName, dexArguments.network);
    console.debug('dexData', dexData);

    const tokenIn = await this.providerService.fetchTokenDetails(
      dexArguments.tokenInAddress,
      dexData.network,
    );
    const tokenOut = await this.providerService.fetchTokenDetails(
      dexArguments.tokenOutAddress,
      dexData.network,
    );
    ...
    const isFulfilled = await this.checkPreconditions(
      dexData,
      createTradeDto,
      instance,
    );
  ...
  async checkPreconditions(
    ...
    console.info(
      `calling allowance on permit2 contract [${permit2Contract.address}], args: ${createTradeDto.receiverAddress}, ${createTradeDto.tokenIn.address}, ${dex.routerAddress}`,
    );
    const allowance = await permit2Contract.allowance(
      createTradeDto.receiverAddress,
      createTradeDto.tokenIn.address,
      dex.universalRouterAddress,
    );
    if (allowance < createTradeDto.tokenInAmount) {
      console.error(
        `Insufficient allowance for ${createTradeDto.tokenInAmount} ${createTradeDto.tokenIn.symbol}`,
      );
      ...
      console.info(
        `Encoding appprove for permit2 [${permit2Contract.address}], args: ${createTradeDto.tokenIn.address} ${dex.universalRouterAddress} ${maxAmount}, ${maxExpiration}`,
      );
      const methodData = permit2Contract.interface.encodeFunctionData(
        'approve',
        [
          createTradeDto.tokenIn.address,
          dex.universalRouterAddress,
