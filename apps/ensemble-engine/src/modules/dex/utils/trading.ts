...
import { UniswapTrade, SwapRouter } from '@uniswap/universal-router-sdk';
...
export class CreateTrade {
  ...
  async create(createTradeDto: CreateTradeDto) {
    ...
    console.log(
      'createTradeDto.tokenIn.decimals',
      typeof createTradeDto.tokenIn.decimals,
    );
    ...
    const swapRoute = new Route([pool], TOKEN_IN, TOKEN_OUT);
    ...
    const routerTrade = new UniswapTrade(
      new RouterTrade({
        v3Routes: [{ routev3: swapRoute, outputAmount, inputAmount }],
        tradeType: TradeType.EXACT_INPUT,
      }),
      options,
    );
  ...
  async getOutputQuote(
    ...
    console.log(
      'rawAmount',
      CurrencyAmount.fromRawAmount(tokenIn, amountIn).toFixed(),
    );
