...
export class SnapshotBuilderService {
  ...
  async buildSnapshotFromBlock(
    ...
    console.log(
      `building snapshot from block ${snapshotArguments.startBlock}, on ${workflowContext.network}`,
    );
    // const previousSnapshot = await this.snapshotModel.findOne().sort({ timestamp: -1 }).exec();
    const latestBalances = await this.balancesService.getLatestBalances(
    ...
    if (startBlock > latestBlock) {
      console.warn(
        `startBlock ${startBlock} is greater than latestBlock ${latestBlock}, skipping snapshot creation`,
      );
      return;
    }
    const toBlock = latestBlock;
    // Fetch all ERC20 transfers from blockNumber
    const transfers = await this.fetchERC20Transfers(
    ...
    console.info(
      `Snapshot to be created properties: tokenAddress: ${snapshotArguments.tokenAddress}, metwork: ${snapshotArguments.network}, blockNumber: ${snapshotArguments.startBlock}`,
    );
    ...
    for (const transfer of transfers) {
      ...
      const updatedSenderBalance = await this.balancesService.update(
        from,
        -value,
        newSnapshot._id.toString(),
        snapshotArguments,
      );
      updatedBalances.push(updatedSenderBalance);

      // Update receiver's balance
      const updatedReceiverBalance = await this.balancesService.update(
        to,
        value,
        newSnapshot._id.toString(),
        snapshotArguments,
    ...
    for (const holder of tokenHolders.filter(
      (holder) =>
        !updatedBalances.some(
          (ub) => ub.accountAddress === holder.accountAddress,
        ),
    )) {
  ...
  private async fetchERC20Transfers(
    ...
    while (true) {
      ...
      console.log(
        `fetching ERC20 transfers from block ${currentBlock} to ${toBlockTmp}, on ${workflowContext.network}`,
      );
      const fetchedLogs = await provider.getLogs(filter);
      console.log(
        `fetched ${fetchedLogs.length} logs from block ${currentBlock} to ${toBlockTmp}, on ${workflowContext.network}`,
      );
