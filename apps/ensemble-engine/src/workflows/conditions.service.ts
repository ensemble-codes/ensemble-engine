...
export class ConditionsService {
  ...
  async fetchEventCondition(
    ...
    const eventSignature = await this.providerService.loadEvent(
      trigger.contract,
      trigger.event,
      contracts,
    );
    console.debug(
      `event condition trigger ${trigger.name}. event: ${trigger.event}, fromBlock: ${fromBlock}`,
    );
    const provider = this.providerService.getProvider(network);
    const toBlock = await provider.getBlockNumber();
    const logs = await this.providerService.fetchEvents(
      contract,
      network,
      eventSignature,
      fromBlock,
